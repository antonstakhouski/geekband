...
/* Bluetooth module*/
SoftwareSerial BTSerial(2, 3); //Bluetooth (TX, RX) settings
...
void setup() {
	...
	//Bits:
  //0-2 -clksel(0 - internal 8MHz oscillator)
  //3 - Temp_DIS
  //4 - --
  //5 - CYCLE
  //6 - SLEEP
...
void loop() {
  ...
  if(curSensoredTime - prevSensoredTime > SENSOR_READ_INTERVAL) {
    ...
    if(iAccelIndex >= ACCEL_BUFFER_COUNT - 3) {
      sendToRemote();
      initBuffer();
      Serial.println("------------- Send 20 accel and temp data to remote");
    }
...
void sendToRemote() {
  ...
  BTSerial.write( aAccelBuffer, ACCEL_BUFFER_COUNT );
}
...
void readFromSensor() {
  ...
  if(iAccelIndex < ACCEL_BUFFER_COUNT && iAccelIndex > 1) {
    ...
    */
    ...
    // as in RM file
    temperature = ( (double) accel_t_gyro.value.temperature) / 340.0 + 36.53;
